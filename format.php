...
//$PAGE->requires->js('/course/format/bootstraptabs/js/tab.js');
//$PAGE->requires->js('/course/format/bootstraptabs/module.js');

//echo '<script src="' . $CFG->wwwroot . '/course/format/bootstraptabs/js/tab.js"></script>';
//echo '<script src="' . $CFG->wwwroot . '/course/format/bootstraptabs/module.js"></script>';
...
$bootstraptabsrenderer = $PAGE->get_renderer('format_bootstraptabs');
...
//$jsmodule = array(
//    'name' => 'weekstabs',
//    'fullpath' => '/course/format/bootstraptabs/module.js',
...
/* <![CDATA[ */
@import url("' . $CFG->wwwroot . '/course/format/bootstraptabs/styles.css");
/* ]]> */
</style>
';
    echo '
    ...
    $sectionmenu = array();
    $num = $start_tab;
    echo '<div id="sections-tabs">'; //this is the first div that yui looks at, top node

    //echo '<ul class="nav nav-tabs" role="tablist">'; //begining of the unordered list
...
//                echo '<li id ="marker" class="markerselected"><a href="#section-' . $section . '" aria-controls="section-' . $section . '" role="tab" data-toggle="tab" id = "marker" class="markerselected">' . $secname . '</a></li>';
//            else
//                echo '<li role="presentation"><a href="#section-' . $section . '" aria-controls="section-' . $section . '" role="tab" data-toggle="tab">' . $secname . '</a></li>'; //prints each sectio
//        }
        $section++;
    }
    //echo '</ul>';

    echo '<div class="tab-content">'; //should be the div for content.
    ...
    while ($section <= $course->numsections)
    {
        ...
        if (!$user_access && $unaval_override)
        {
            ...
            echo '<div class="content">';
            echo $bootstraptabsrenderer->section_hidden($section);
            echo '</div>';
            echo '</div>';
        ...
        if ($showsection)
        {
            ...
            if (has_capability('moodle/course:viewhiddensections', $context) || $thissection->visible)
            {
                if($section == 1){
                    ...
                    echo '<div role="tabpanel" class="tab-pane fade" id="section-' . $section . '">';
                }
                ...
                echo $bootstraptabsrenderer->section_availability_message($thissection, has_capability('moodle/course:viewhiddensections', $context));
    ...
    //$PAGE->requires->js_init_call('M.bootstraptabs.init', array($course->id, $isZeroTab), false, $jsmodule);
